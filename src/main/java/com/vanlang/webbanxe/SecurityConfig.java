...
public class SecurityConfig {
    private final AccountService accountService; // Tiêm UserService vào lớp cấu hình này.


    @Bean // Đánh dấu phương thức trả về một bean được quản lý bởi Spring Context.
    public UserDetailsService userDetailsService() {
        return new AccountService(); // Cung cấp dịch vụ xử lý chi tiết người dùng.
    }
    @Bean
    ...
    public SecurityFilterChain securityFilterChain(@NotNull HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/css/**", "/js/**", "/", "/oauth/**", "/register", "/error", "/products", "/cart", "/cart/**","/upload/**","/asset/**")
